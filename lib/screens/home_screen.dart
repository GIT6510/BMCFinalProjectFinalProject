...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // 3. Lazy load flag for user role
  bool _userRoleFetched = false;

  // 4. This function runs ONCE when the screen is first created
  @override
  void initState() {
    super.initState();
    // 5. Defer user role fetch to avoid blocking initial render
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _fetchUserRole();
    });
  }

  // 6. This is our new function to get data from Firestore
  Future<void> _fetchUserRole() async {
    // 7. If no one is logged in or already fetched, do nothing
    if (_currentUser == null || _userRoleFetched) return;
    try {
      // 8. Go to the 'users' collection, find the document
      ...
      // 9. If the document exists...
      if (doc.exists && doc.data() != null) {
        // 10. ...call setState() to save the role to our variable
        setState(() {
          _userRole = doc.data()!['role'];
          _userRoleFetched = true;
        });
      }
    } catch (e) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        */
        ...
        title: Image.asset(
          'assets/images/comick-logo.png', // 3. The path to your logo
